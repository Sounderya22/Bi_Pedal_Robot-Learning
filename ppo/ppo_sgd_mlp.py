...
def traj_segment_generator(pi, gamma, env, horizon, stochastic):
    ...
    obs_pol = np.array([ob_pol for _ in range(horizon)])
...
def learn(
    ...
    *,
    ...
    pi = policy_fn(
        "pi", ob_space_vf, ob_space_pol, ac_space
    )  # Construct network for new policy
    oldpi = policy_fn(
        "oldpi", ob_space_vf, ob_space_pol, ac_space
    ...
    lossandgrad = U.function(
        [ob_vf, ob_pol, ac, atarg, ret, lrmult],
    ...
    compute_losses = U.function(
        [ob_vf, ob_pol, ac, atarg, ret, lrmult], [losses, clip_frac]
    )
    ...
    if continue_from:
        latest_model = tf.train.latest_checkpoint("../tf_model/" + continue_from)
    ...
    while True and max_iters != 1:
        ...
        # todo
        ob_vf, ob_pol, ac, atarg, tdlamret = (
            seg["ob_vf"],
            seg["ob_pol"],
            seg["ac"],
            seg["adv"],
        ...
        d = Dataset(
            dict(ob_vf=ob_vf, ob_pol=ob_pol, ac=ac, atarg=atarg, vtarg=tdlamret),
